...
exports.buildImg = function(opt, cb) {
  var imgPath = path.join(opt['img_src'], '**');
...
builder.css = function(file, opt, cb) {
  return gulp.src(file)
    .pipe(gi(/\.less$/, less()))
    .pipe(plugin.cssParser(opt))
    .pipe(gulp.dest(opt.dest))
    .pipe(gi(use('cssimg', opt), css.cssimg.gulp()))
    .pipe(gi(use('peaches', opt), peaches()))
...
builder.js = function(file, opt, cb) {
  ...
  return umi.src(file, opt)
    ...
    // seajs transport
    .pipe(gi(/\.css$/, plugin.cssParser(opt)))
    .pipe(gi(/\.css$/, plugin.css2jsParser(opt)))
    .pipe(gi(/\.tpl$/, plugin.tplParser(opt)))
    .pipe(gi(/\.json$/, plugin.jsonParser(opt)))
    .pipe(gi(/\.handlebars$/, plugin.handlebarsParser(opt)))
    .pipe(gi(/\.js$/, plugin.jsParser(opt)))
    .pipe(umi.concat())
...
function getHeader(file, opt) {
  ...
  var header = [
    ...
    fs.readFileSync(path.join(__dirname, './template/build/sea-style-debug.js'), 'utf-8')
  ];
  return header.join('\n');
}
